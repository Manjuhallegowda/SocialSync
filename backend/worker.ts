...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (path === '/api/auth/login' && method === 'GET') {
        ...
        const metaUrl = `https://www.facebook.com/v19.0/dialog/oauth?client_id=${env.META_APP_ID}&redirect_uri=${env.META_REDIRECT_URI}&state=${encodeURIComponent(state)}&scope=pages_show_list,pages_read_engagement,pages_manage_posts,instagram_basic,instagram_content_publish&response_type=code`;
      ...
      if (path === '/api/auth/callback' && method === 'GET') {
        ...
        if (error || !code) {
             return Response.redirect(`${frontendUrl}/${redirectBase}?error=auth_failed`, 302);
        }
        
        // Exchange code for token
        const tokenResp = await fetch(`https://graph.facebook.com/v19.0/oauth/access_token?client_id=${env.META_APP_ID}&redirect_uri=${env.META_REDIRECT_URI}&client_secret=${env.META_APP_SECRET}&code=${code}`);
        const tokenData: any = await tokenResp.json();
        
        if (!tokenData.access_token) {
             return Response.redirect(`${frontendUrl}/${redirectBase}?error=token_failed`, 302);
        }

        // Get Long Lived Token
        const longTokenResp = await fetch(`https://graph.facebook.com/v19.0/oauth/access_token?grant_type=fb_exchange_token&client_id=${env.META_APP_ID}&client_secret=${env.META_APP_SECRET}&fb_exchange_token=${tokenData.access_token}`);
        const longTokenData: any = await longTokenResp.json();
        const finalToken = longTokenData.access_token || tokenData.access_token;
        const expiry = Date.now() + (longTokenData.expires_in ? longTokenData.expires_in * 1000 : 5184000000); // 60 days default

        // Fetch Accounts using the User Token
        const accountsResp = await fetch(`https://graph.facebook.com/v19.0/me/accounts?fields=name,access_token,instagram_business_account&access_token=${finalToken}`);
        const accountsData: any = await accountsResp.json();

        let connectedCount = 0;
        const stmt = env.DB.prepare(`INSERT OR REPLACE INTO accounts (id, owner_name, fb_page_id, fb_page_name, ig_user_id, access_token, token_expires_at, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        ...
        if (accountsData.data) {
          for (const page of accountsData.data) {
            ...
            if (page.instagram_business_account) {
              ...
              const encryptedPageToken = await encryptToken(page.access_token, secretKey);
        ...
        return Response.redirect(`${frontendUrl}/${redirectBase}?success=true&count=${connectedCount}`, 302);
      }

      // 3. GET ACCOUNTS
      if (path === '/api/accounts' && method === 'GET') {
        const { results } = await env.DB.prepare('SELECT id, owner_name, fb_page_name, ig_username, status, last_updated FROM accounts ORDER BY created_at DESC').all();
      ...
      if (path === '/api/verify-user' && method === 'POST') {
          ...
          const user = await env.DB.prepare('SELECT * FROM allowed_users WHERE lower(username) = lower(?)').bind(username).first();
          
          if (user) {
              return new Response(JSON.stringify({ verified: true, username: user.username }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
          } else {
      ...
      if (path === '/api/allowed-users' && method === 'GET') {
          const { results } = await env.DB.prepare('SELECT * FROM allowed_users ORDER BY created_at DESC').all();
          return new Response(JSON.stringify(results), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      if (path === '/api/allowed-users' && method === 'POST') {
          ...
          try {
              await env.DB.prepare('INSERT INTO allowed_users (id, username, created_at) VALUES (?, ?, ?)')
                .bind(crypto.randomUUID(), username.trim(), Date.now())
                .run();
              return new Response(JSON.stringify({ success: true }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
          } catch (e: any) {
              // Likely unique constraint violation
              return new Response(JSON.stringify({ success: false, error: 'User likely already exists' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
          }
      }
      ...
      if (path === '/api/allowed-users/bulk' && method === 'POST') {
          ...
          if (!Array.isArray(usernames)) return new Response('Invalid data', { status: 400 });

          const stmt = env.DB.prepare('INSERT OR IGNORE INTO allowed_users (id, username, created_at) VALUES (?, ?, ?)');
          const batch = usernames.map((name: string) => stmt.bind(crypto.randomUUID(), name.trim(), Date.now()));
          
          if (batch.length > 0) await env.DB.batch(batch);

          return new Response(JSON.stringify({ success: true, count: batch.length }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
  ...
  async scheduled(event: any, env: Env, ctx: any): Promise<void> {
    ...
    console.log(`[Cron] Starting token refresh check. Looking for expiry < ${new Date(threshold).toISOString()}`);
  ...
  async queue(batch: any, env: Env): Promise<void> {
    for (const message of batch.messages) {
      const { type, postId, accountId, currentAccessToken, accounts, post, retryCount } = message.body;
  ...
  async handleRefreshToken(accountId: string, encryptedCurrentToken: string, env: Env) {
     const secretKey = env.ENCRYPTION_KEY || env.API_SECRET;
     try {
       // Decrypt token to use it
       const decryptedToken = await decryptToken(encryptedCurrentToken, secretKey);
       ...
       if (data.error || !data.access_token) {
         throw new Error(data.error?.message || 'Token exchange returned no token');
       }
  ...
  async handleStartPost(postId: string, env: Env) {
    ...
    const { results: accounts } = await env.DB.prepare('SELECT * FROM accounts WHERE status = ?').bind('active').all();
    
    // Update total count and status to 'in_progress'
    await env.DB.prepare('UPDATE posts SET total_accounts = ?, status = ? WHERE id = ?')
  ...
  async handleRetryAccountPost(postId: string, accountId: string, retryCount: number, env: Env) {
     const post = await env.DB.prepare('SELECT * FROM posts WHERE id = ?').bind(postId).first();
     const account = await env.DB.prepare('SELECT * FROM accounts WHERE id = ?').bind(accountId).first();
     
     if (!post || !account) {
         console.error("Retry failed: Data not found for post/account", postId, accountId);
         return;
     }
  ...
  async attemptPost(account: any, post: any, secretKey: string, env: Env, retryCount: number) {
      // Apply Anti-Spam Variations (Shuffle hashtags, inject emojis)
      const finalCaption = processCaption(post.base_caption, account.fb_page_name);
      
      try {
          const metaIds = await this.publishToMeta(account, post, finalCaption, secretKey);
          
          // Log Success
          await env.DB.prepare('INSERT INTO logs (post_id, account_id, final_caption, meta_post_id, status, timestamp) VALUES (?, ?, ?, ?, ?, ?)')
            .bind(post.id, account.id, finalCaption, metaIds, 'success', Date.now())
            .run();
        
          await env.DB.prepare('UPDATE posts SET success_count = success_count + 1 WHERE id = ?').bind(post.id).run();
          ...
          if (retryCount < MAX_RETRIES) {
               ...
               console.log(`[Retry] Queuing retry #${nextRetry} for ${account.fb_page_name} (Delay: ${backoffSeconds}s). Reason: ${err.message}`);
               ...
               // Fail
               console.error(`[Failure] Max retries reached for ${account.fb_page_name}.`);
               
               await env.DB.prepare('INSERT INTO logs (post_id, account_id, final_caption, error_message, status, timestamp) VALUES (?, ?, ?, ?, ?, ?)')
                .bind(post.id, account.id, finalCaption, `Max Retries Exceeded: ${err.message}`, 'failed', Date.now())
                .run();
                
               await env.DB.prepare('UPDATE posts SET failure_count = failure_count + 1 WHERE id = ?').bind(post.id).run();
          }
      }
  },

  async publishToMeta(account: any, post: any, caption: string, secretKey: string): Promise<string> {
        // Decrypt Token
        let accessToken;
        try {
        ...
        if (containerData.error || !containerData.id) {
            throw new Error('IG Container: ' + (containerData.error?.message || 'Unknown error'));
        }
        ...
        if (publishData.error || !publishData.id) {
            throw new Error('IG Publish: ' + (publishData.error?.message || 'Unknown error'));
        }
  ...
  async checkCampaignCompletion(postId: string, env: Env) {
    const stats = await env.DB.prepare('SELECT success_count, failure_count, total_accounts FROM posts WHERE id = ?').bind(postId).first();
    if (stats && (stats.success_count + stats.failure_count >= stats.total_accounts)) {
       await env.DB.prepare("UPDATE posts SET status = 'completed', completed_at = ? WHERE id = ? AND status != 'completed'")
